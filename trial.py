...
def trackbary_init(title):
    ...
    cv.setTrackbarPos('THR', title, 172)
...
def main_automatic():
    ...
    values_white, values_pink, values_red = loading_data('Values_trial')
    ...
    while (capture.isOpened()):
        ...
        for cnt in contours:
            ...
            if (30 > len(approx) >= 5) and (170 >area > 50) and (0.8 < (w / h) < 1.2) and (0.5*0.5*w*1.2 > area/peri > 0.5*0.5*w*0.8):
                if w>h:
                    vector.append([area, w, x, y, peri])
                else:
                    vector.append([area, h, x, y, peri])
        
        vector = vector[0:4]
        vector = sorted(vector, key=take_third, reverse=True)
        #print('len:',len(approx),'area', area,'peri:',peri,'w/h', w/h, 'Pp/L', area/peri)
        for area, w, x, y, peri in vector:
            print('H_r:', round(0.5*0.5*w*1.2,2), 'R', 0.5*0.5*w, 'H_l:',round(0.5*0.5*w*0.8,2))
        #print(vector)
        ...
        if len(vector) == 4:
            ...
            Red = ((find_Color_automatic(crop_Red[0], 0.3),
                    find_Color_automatic(crop_Red[1], 0.3),
                    find_Color_automatic(crop_Red[2], 0.3),
                    find_Color_automatic(crop_Red[3], 0.3)))

            White = ((find_Color_automatic(crop_White[0], 0.5),
                    find_Color_automatic(crop_White[1], 0.5),
                    find_Color_automatic(crop_White[2], 0.5),
                    find_Color_automatic(crop_White[3], 0.5)))
        ...
        for area, w, x, y, peri in vector:
            cv.rectangle(frame, (int(x - 0.6 * w), int(y - 0.6 * h)), (int(x + 0.6 * w), int(y + 0.6 * h)),
                            (255, 0, 0), 1)
